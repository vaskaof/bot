...
/**
 * Полное объяснение системы Сов/Билетов — для карточки баланса на Профиле,
 * единственном месте, где уместен полный текст целиком (это и есть экран
 * "мой баланс"). Текст согласован с VASY в чате 19.08.2026. Реферальная
 * строка ("вам — N сов...") намеренно НЕ дублируется здесь отдельным
 * числом — та же информация уже видна как обычный читаемый текст в блоке
 * "Пригласите друга" чуть ниже на этом же экране.
...
/**
 * Короткое объяснение НА КОНКРЕТНОЙ карточке заказа, рядом со строкой
 * "🎟️ N сов начислено"/"+N сов при полной оплате" — контекст: "как именно
 * посчитана вот эта цифра", не вся система целиком.
 * @param {{conversionRateRub:number, sovyPerTicket:number}} info
 * @returns {string}
 */
function buildSovyOrderNote(info) {
    return `<p>Совы начисляются автоматически, как только заказ будет полностью оплачен — сейчас в среднем ${info.conversionRateRub} ₽ оплаты ≈ ${info.sovyPerTicket} Сов (курс может измениться). Если этот заказ будет удалён, начисление отменится.</p>`;
}

/**
 * Короткое объяснение НА КОНКРЕТНОЙ карточке задания, рядом со строкой
 * "+N сов" — без живых чисел курса (награда уже видна в самой карточке),
 * фокус на том, когда она реально начисляется.
 * @returns {string}
 */
function buildSovyTaskNote() {
    return `<p>Награда начисляется после того, как менеджер проверит и одобрит выполнение. Для некоторых заданий, если условие выполнения позже перестаёт быть верным, начисленные Совы могут быть списаны обратно.</p>`;
}
