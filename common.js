...
/**
 * Заголовок общей подсказки "?" про экономику сов/билетов — SOVY_HELP_TITLE
 * используется как canonical title во всех местах, где вызывается
 * buildSovyHelpBody, чтобы не расходились формулировки по экранам.
 */
const SOVY_HELP_TITLE = '🦉 Как работает система Сов';

/**
 * Общий текст клиентской подсказки "?" про экономику сов/билетов — единый
 * источник для Профиль/Конкурсы/Заказы (19.08.2026, п.2 бета-фидбека,
 * текст согласован с VASY в чате). Раньше в profile.js/contests.js жил
 * ЗАДУБЛИРОВАННЫЙ статичный текст с зашитыми числами ("в среднем 5000 ₽ ≈
 * 100 сов", "Каждые 100 Сов = 1 Билет") — расходился с реальностью с
 * 18.08.2026, когда "Сов_За_Билет" стало редактируемым в админке. Числа
 * теперь живые — приходят из getReferralInfo (conversionRateRub/
 * sovyPerTicket/inviteeReward/referrerReward), тот же вызов уже нужен
 * экранам с балансом/рефералкой; на "Заказы", где своего вызова
 * getReferralInfo нет, экран делает отдельный лёгкий запрос сам (см.
 * client/screens/orders.js) — числа не хранятся модулем, чтобы правка
 * курса в "Настройках" отражалась немедленно, без кэша на фронте.
 * @param {{conversionRateRub:number, sovyPerTicket:number, inviteeReward:number, referrerReward:number}} info
 * @returns {string}
 */
function buildSovyHelpBody(info) {
    return `
