  ...
  init({ onApplied, getStatusDictionary }) {
    ...
    applyBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        // Баг 23.09.2026 (репорт VASY, "смена статуса зависает, если есть
        // долг") — когда ВСЕ выбранные заказы блокированы (типично для
        // автоматизации коллективки: весь этап целиком ещё не оплачен),
        // `trulyReady` пуст, ветка выше со `showConfirmModal` не выполняется
        // вовсе — раньше это означало ПОЛНУЮ тишину: ни тоста, ни
        // подтверждения, только список ниже вместо кнопки "Применить".
        // Менеджер (особенно в авто-вызове из `collective-detail.js`, где
        // следом ещё всплывает несвязанный тост "Коллективка обновлена")
        // не понимал, что действие вообще что-то сделало — выглядело как
        // зависшее. Явный тост здесь — тот же принцип, что уже есть у
        // ошибки запроса чуть ниже, просто для этого конкретного "0 из N".
        if (trulyReady.length === 0) {
          showSaveToast(false, `Ни один из ${orderIds.length} заказ(ов) не готов к переходу автоматически — все требуют подтверждения ниже («Всё равно перевести»).`);
        }
