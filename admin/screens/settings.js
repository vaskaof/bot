...
const ECONOMY_SETTINGS_DEFS = [
  ...
  // Э7 (26.08.2026) — НЕ то же самое, что курс начисления выше: во сколько
  // сама компания оценивает билет как обязательство для проводок леджера
  // (счета 2100/5400), не то, сколько клиент потратил, чтобы его заработать.
  // 0 = "не оценён" — проводки не пишутся, см. ticketEconomyService.js.
  { key: 'Билет_Оценка_Руб', label: 'Оценка билета для учёта, ₽', min: 0 },
  // Разовый пул с ручным пополнением (не помесячный автосброс) — дашборд
  // "Билеты/Конкурсы" на экране "Кошелёк ₸". 0 = бюджет не задан.
  { key: 'Бюджет_Маркетинг_Конкурсы', label: 'Бюджет на конкурсы, ₽', min: 0 }
];
